...
## 2026-05-18 演示数据库重置
- 本次重置使用受控演示数据库；连接信息只通过本机环境变量或被 Git 忽略的本地配置提供，文档不记录真实地址或凭据。
- 重置前已在被 Git 忽略的本地工作目录中完成数据库备份，备份文件不进入仓库。
...
- 新种子脚本存放在被 Git 忽略的本地工作目录；导入前先创建临时校验库跑同一份表结构和种子脚本，验证通过后再导入目标演示库。
...
- 后端主配置不能保存真实数据库地址或密码；已改为通过环境变量拼接数据库连接，支持 `SERVER_PUBLIC_IP`、`DB_PORT`、`DB_PASSWORD`，并兼容 `DB_HOST`、`MYSQL_HOST`、`DB_USERNAME`、`MYSQL_USER` 等通用变量名。按用户当前本机配置，数据库用户名未设置时默认 `root`。
- 新增 `application-example.yml` 只提供本地示例值，不包含生产凭据。
- 根目录和后端 `.gitignore` 已补充本地 Spring 配置、secret 配置、dump/backup 压缩包和数据库备份目录规则。
- 当前旧数据库连接信息仍存在于 Git 历史提交中，属于后续“Git 历史清理”步骤处理范围。
- 本机启动使用被 Git 忽略的 `application-local.yml`，从用户级环境变量读取数据库 host、port 和 password；`start-server-8081.cmd` 会在当前进程缺少这些变量时从 Windows 用户级/机器级环境变量补读。

## 2026-05-18 发布前文档敏感信息清理
- 后端上下文文档中的真实数据库地址、具体备份文件名和直接目标库操作表述已替换为受控演示库、目标演示库和被 Git 忽略本地工件描述。
- README 和中文 README 保留演示账号，但明确这些凭据只适用于本地演示数据，不能用于公网数据库或部署环境。
- 完整功能测试清单保留测试账号矩阵，因为它是手工验收输入；已增加默认账号只用于本地或受控演示数据的安全说明。
- 当前文件层面不再应出现旧真实数据库地址、备份文件名或种子 SQL 文件名；Git 历史仍需后续单独清理。

## 2026-05-18 发布前许可证与历史风险梳理
- 根目录已新增 MIT `LICENSE`，README 和 README.zh-CN 改为声明当前仓库采用 MIT 协议。
- `frontend/LICENSE` 继续保留上游模板 MIT 版权声明，这是许可证合规要求，不应删除。
- Git 历史扫描确认旧提交曾包含已删除的数据库连接信息和旧默认密码；用户确认后已在后续阶段通过重写历史清理。
- 历史重写会改变提交哈希并影响远端协作者；本仓库已使用 `--force-with-lease` 推送，避免覆盖远端新提交。

## 2026-05-18 Git 历史敏感信息清理
- 已提交发布前清理结果，并用 `git-filter-repo --replace-text` 重写本地历史。
- 替换范围仅限旧数据库地址、旧明文数据库密码配置行、旧备份 SQL 文件名和旧种子 SQL 文件名；未全局替换演示账号密码。
- 历史重写后重新添加 `origin`，使用 `--force-with-lease` 将 `master` 推送到 GitHub。
- 推送后本地与 `origin/master` 一致，全历史扫描旧敏感字符串无匹配。
